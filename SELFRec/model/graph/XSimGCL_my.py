...
class XSimGCL_my(GraphRecommender):
    def __init__(self, conf, training_set, test_set):
        super(XSimGCL_my, self).__init__(conf, training_set, test_set)
        ...
        self.model = XSimGCL_Encoder(self.data, self.emb_size, self.eps, self.n_layers,self.layer_cl, self.scale)
...
class XSimGCL_Encoder(nn.Module):
    def __init__(self, data, emb_size, eps, n_layers, layer_cl, scale):
    ...
    def forward(self, perturbed=False):
        ...
        item_all_embeddings_cl = F.normalize(item_all_embeddings_cl, dim=-1, p=2) * self.scale
